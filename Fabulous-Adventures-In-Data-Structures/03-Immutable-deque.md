...
想象一下一个树形数据结构：有一个根节点，一些节点是没有子节点的“叶子”，一些节点则有子节点，等等。我们经常会看到这样的树：有些叶子离根节点很远，而有些则相对较浅。如果你从根开始遍历树，那么到达浅层节点的代价就更低。指状树（finger tree）确保某些特定的节点——称为“指针”（fingers）——总是靠近根节点。并且，任何其他节点越接近某个指针，它就越浅。对于一个双端队列来说，我们希望这些指针是树中最左端和最右端的节点；一个节点离两端越近，它就越浅；越接近中间，它就越深。

我们不会直接从头设计指状树数据结构，而是继续不断地攻击“如何构建一个高效的不可变双端队列”这个问题，然后你会看到指状树数据结构是如何自然地从中浮现出来的。
...
为了在左侧和右侧“腾出空间”，我们需要一个辅助的抽象数据类型。我打算把这个 ADT 称为 mini-deque，或者简称 mini。mini 的行为类似 deque：你可以从左边和右边 push 和 pop，但 mini 受到限制：它只能恰好容纳一、二、三或四个元素。不存在空的 mini；尝试对一个只含一个元素的 mini 做 pop，或者对一个已有四个元素的 mini 做 push，都是错误。我们当然可以把 mini-deque 定义成可以容纳多于四个元素，但我们的指状树实现只需要四个，所以就保持在四个即可。
...
我们能否把两个 deque 的连接做到比线性时间更快？在前一章中，我们看到 Hughes 列表为我们提供了两个栈的 O(1) 连接，无论它们的大小是多少，但那有点作弊，因为我们真正做的只是延迟构造连接后的栈，并在之后支付线性代价。幸运的是，我们可以通过稍微弱化我们的不变式，让两个指状树（finger tree）双端队列之间真正做到廉价的连接：我们不再要求每个 mini-deque 都必须是由 Three 组成的 deque，而是要求它可以是由 Three 和/或 Two 组成的 deque。我们可以通过以下更改来实现：
...
放宽不变量以允许在中间出现 Two，会改变我们的性能分析吗？并不会；即使存在一种场景，其中中间双端队列的大部分或全部都是 Two，我们的性能分析也不会有太大变化。

在这种场景下，树的最大深度会从 O(log₃ n) 增加到 O(log₂ n)，但仍然是对数级的。额外的内存依然是每个元素 O(1)。可能出现的“最坏情况”push 和 pop，从平均每三次顶层操作出现一次，增加为平均两次中出现一次。但摊还成本仍然是 push 和 pop 的 O(1)，而最坏情况依然在双端队列规模上是对数级的。我们依然比上一章不可变队列的 O(n) 最坏情况要好得多。

那么连接（concatenation）两个双端队列的性能如何呢？

- 如果任意一个待连接的双端队列为空，那么我们直接返回另一个队列。我们完全不使用额外内存，这显然是 O(1) 时间。
- 如果任意一个双端队列只有单个元素，那么我们把这个单元素 push 到另一个队列上。在这种情况下，连接的成本和 push 一样：平均 O(1) 时间和额外空间，最坏情况是 O(log n) 时间和额外空间，其中 n 为较大队列的规模。
- 辅助方法 `TwosOrThrees` 的成本是 O(1)；它最多只是构造一个长度为三的序列。如果我们正在连接两个多元素双端队列，那么我们需要构造新的 `middle`。首先，我们通过调用 `PushRightMany` 构造一个中间双端队列，把 `TwosOrThrees` 返回的序列中的元素 push 到左侧双端队列的 `middle` 中。每次 push 的摊还时间和额外空间成本都是 O(1)，最坏情况是 O(log n)，其中 n 为左侧队列的规模。接着，我们递归地将中间双端队列连接到右侧双端队列的 `middle` 中。
- 当所有步骤完成后，对两个多元素双端队列进行的连接操作最终会得到一棵深度为 O(log n) 的树，其中 n 是双端队列中元素的总数。最多会有 O(log n) 次递归连接，每次递归执行的操作的摊还成本都是 O(1)，因此连接操作的最坏情况成本是 O(log n) 的时间和额外空间。

总之，放宽不变量并不会在意义上改变渐进性能特征；我们仍然拥有一个高度持久化的数据结构，对 push 和 pop 有低摊还成本，并且其最坏情况成本依旧低且罕见。通过放宽这个不变量，我们还能实现同样低成本的连接操作。

## 小结

- 很容易不小心设计出一个“能用但缺乏持久性”的不可变数据结构，而这样的结构无法做到高效。为了构建一个性能良好的不可变双端队列，我们需要停止依赖链表，转而利用树结构的强大能力。
- Finger Tree 让树中的某些节点——“指针”（fingers）——非常浅，而离它们越远，树就越深。这种数据结构非常适合作为列表使用，因为它是不可变的、持久化的，同时又很便宜。大多数操作的平均成本是 O(1)，而树的深度是 O(log n)，即使是必须修改叶节点的最坏情况，时间和额外内存成本也仅仅是 O(log n)。
- 在设计数据结构时，一个非常有用的技巧是：先以最直接的方式解决问题的一个更受限版本，然后利用该解决方案将问题推广到更大的版本。用简洁直接的代码显式处理所有一到四个元素的双端队列，是解决整体问题的关键一步。
- 具有真正泛型类型系统的语言蕴含大量常常未被充分利用的能力。能够编写在类型上具有无界递归的数据结构，并由编译器和运行时来保证类型安全，是工具箱中一件极其强大的利器。
- 对列表而言，廉价的连接（concatenation）是一种非常好的特性。尽管需要考虑不少情况，但递归连接算法在概念上却十分直观：如果不是平凡情况，就先解决更小的问题——递归地连接中间部分。这在用许多小列表构建大列表时非常有用。
