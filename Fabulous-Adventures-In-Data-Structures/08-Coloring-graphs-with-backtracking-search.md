...
> 为什么不是10种颜色，而是160种？前者是站在登机口的角度，而文章是把空间和时间合并成了一个统一的资源单位。
>
> ### 10种颜色（颜色 = 登机口）
>
> 如果你把 10 个登机口看作 10 种颜色：
>
> - **节点**：每一架飞机（比如“航班 MU5101”）。
> - **边（相邻条件）**：如果两架飞机在**同一个小时**都要用登机口，它们之间就有连边。
> - **挑战**：这种模型只能解决“空间冲突”，不能直接解决“时间调度”。你需要额外写逻辑去处理：第一小时这 10 种颜色怎么分，第二小时这 10 种颜色又怎么分。这更像是在做 16 个独立的图着色问题。
>
> ### 160种颜色（颜色 = 登机口 × 时间点）
>
> 把“资源”彻底细分了。在这个模型里，一种颜色代表一个**“时空坑位”**。 例如：
>
> - 颜色 1：登机口 A 在 08:00 - 09:00
> - 颜色 2：登机口 B 在 08:00 - 09:00
> - ...
> - 颜色 11：登机口 A 在 09:00 - 10:00
> - 总计：10 个口 × 16 个小时 = **160 个唯一的坑位**。
>
> 在这种模型下：
>
> - **节点**：每一架飞机。
...
>   ：这里变得非常灵活！
>
>   - **硬性冲突**：如果两架飞机都在 08:00 到达，它们不能选同一个“时间点”的颜色。
>   - **逻辑约束**：如果一架飞机需要在机场停留 **2 小时**，那么它其实需要占用两个节点，这两个节点选的颜色必须满足“同一个登机口”且“时间连续”。
>   - **优先级**：如果某些大型客机只能停靠在特定的 3 个登机口，那么它的候选颜色集（Palette）就只有这 3 个口对应的 48 种颜色，而不是全部 160 种。
