...
for (int i = 0; i < 9; i += 1)
    sudoku = sudoku.AddClique([.. Enumerable.Range(i * 9, 9)]); #A
for (int i = 0; i < 9; i += 1)
    sudoku = sudoku.AddClique([.. Enumerable.Range(0,9).Select(x => x * 9 + i)]); #B
sudoku = sudoku
    .AddClique([00, 01, 02, 09, 10, 11, 18, 19, 20]) #C
...
> 每个节点的度都是 20 是怎么得出来的？
>
> 首先要搞清一个误区：图着色和数独实际上属于同一类问题。当我们看到一个网格时，第一反应确实是它的“物理空间相邻”——也就是它上下左右以及对角线上的 8 个邻居。
>
> 但是，在将**数独转化为图着色问题**时，我们对“相邻（连边）”的定义发生了根本性的变化。在这里，**“两个节点之间有一条边” 并不意味着它们在物理位置上挨在一起，而是意味着“根据数独的规则，它们绝对不能填入相同的数字”。**
>
> 所以熟读的“图着色”的相邻规则在数独的场景下就演变成了数独的规则：**数独的规则是：每一行、每一列、每一个 3×3 的宫格内，数字不能重复。**
>
> 我们随便挑一个格子（假设叫它格子 A），看看它受多少个格子的约束：
>
> 1. **同一行的约束：** 数独一行有 9 个格子。除了格子 A 自己，还有 **8** 个格子。格子 A 不能和这 8 个格子填一样的数字。（贡献度数：**8**）
> 2. **同一列的约束：** 数独一列也有 9 个格子。除了格子 A 自己，同行交叉的那 8 个已经算过了。现在同一列里，除了 A 自己，还有 **8** 个格子。（贡献度数：**8**）
> 3. **同一宫格（3×3）的约束：** 一个 3×3 宫格里总共有 9 个格子。这 9 个格子中：
>    - 1 个是格子 A 自己。
>    - 2 个格子和 A 在同一行（刚才在第1步算行的时候已经数过了）。
>    - 2 个格子和 A 在同一列（刚才在第2步算列的时候已经数过了）。
>    - 那么，宫格里还剩下几个格子没被数过呢？ 9 - 1(自己) - 2(同行) - 2(同列) = **4** 个格子。 格子 A 也不能和这 4 个格子填一样的数字。（贡献度数：**4**）
>
> 把它们加起来： **8 (行) + 8 (列) + 4 (宫格剩余) = 20**
...
如果你做过报纸上的数独题，你就会知道，猜测—回溯算法并不是人类解这些谜题的方式。数独爱好者把这种方法称为“二分（bifurcation）”算法，因为当你真的卡住时，有时会选择一个只有两个可能数字的格子，把两种可能性都试一遍。二分法在数独圈子里名声不佳；几乎总能找到一种更优雅的方式来推进。我们的 reducer 能不能更快、更聪明一些？我们错过了一些技巧：

reducer 从不检查当前尝试是否已经被破坏。它在图中到处寻找只有单一颜色的节点来进行约简，但如果某个新产生的约简把一个节点减少到没有任何剩余可能颜色，它仍然会继续工作。它依赖 backtracker 来检测并过滤掉这些被破坏的尝试。

当 reducer 第一次调用 ReduceOnce 时，它会查看每一个节点，找出哪些节点只有一种颜色，然后查看所有相邻节点，看是否需要从它们那里移除该颜色。到了第二次调用 ReduceOnce，几乎所有这些工作都会再做一遍！reducer 本可以更聪明一些，记录之前调用 ReduceOnce 时已经完成了哪些工作，并在后续调用中避免重复。

reducer 完全不会对团（clique）进行推理，但一个节点数与可用颜色数相同的团，往往蕴含着约简的机会。再看看那个示例谜题。底部中间的宫里一定有一个 7。它在哪里？一个更好的 reducer 可以在第一次调用 Reduce 时就注意到，这个团中的某个节点必然是 7，然后基于这一新事实继续进行更多的约简。

数独爱好者有几十种技巧，能够利用数独谜题中大量存在的团，在不使用二分法的情况下，迅速减少每个格子的可能性。所有这些技巧都可以被编码进一个图着色 reducer 中，从而极大地加速回溯算法。

我不打算实现这些改进中的任何一个；图着色是一个被深入研究的问题，有许多特殊情况可以被检测并加以利用，但我暂时就讲到这里。

### 8.5.4 回溯与笛卡尔积

我们已经看到，当存在大量约束、且 reducer 可以加以利用时，回溯算法表现得很好。那么相反的情况呢——当一个着色问题几乎没有任何约束时会怎样？假设我们有两个彼此完全不相邻的岛屿，以及四种可能的颜色：
...
foreach (var s in islandsBT.Solve(islandsInit))
    Console.WriteLine(s["A"].Single().Name + " " + s["B"].Single().Name);
```

当没有任何约束时，可能的解就会非常多；事实上一共有十六种：

```
(Yellow,Yellow) (Yellow,Red) (Yellow,Green) (Yellow,Purple) (Red,Yellow)
(Red,Red) (Red,Green) (Red,Purple) (Green,Yellow) (Green,Red) (Green,Green)
(Green,Purple) (Purple,Yellow) (Purple,Red) (Purple,Green) (Purple,Purple)
```

你可能已经注意到，这正是由四种颜色构成的集合与其自身的笛卡尔积。我们在第 6 章中已经知道，可以通过组合多次 SelectMany 调用来计算笛卡尔积；而这正是我们的回溯算法在做的事情。

回溯搜索本质上是在枚举可能解的笛卡尔积，同时利用约简步骤，尽可能早地过滤掉那些已经被破坏的尝试。

我们通常把“搜索问题”理解为这样的情形：你有一张采购订单表，正在寻找某个特定客户的订单；或者你有一棵树，正在向下遍历以寻找某个特定节点。回溯算法解决的是一种更抽象的搜索问题：存在一个可能解的空间，而我们正在探索这个空间，寻找那些满足要求的尝试。约简步骤的作用，在于识别出我们知道不可能导向解的空间部分，并将其避开。

我们已经看到，可以用图着色来解决一些实际问题，但也许数独并不是一个特别有说服力的例子。还有哪些问题可以用这个工具来解决呢？

## 8.6 调度问题就是图着色问题

给地图着色、解数独都是实际问题，但它们多少有些趣味性。图着色之所以被如此深入地研究，并不仅仅因为它在抽象数学中引人入胜，更因为工业界中的许多现实问题都可以建模为图着色问题，例如调度问题：

一天只有那么多小时，一个机场也只有那么多登机口。如果一个拥有 10 个登机口的机场每天开放 16 个小时，而每架飞机在登机口停留一小时，那么我们就可以构造 160 种“颜色”，每一种对应某个登机口的某个小时。飞机可以作为图中的节点，如果两架飞机在同一小时到达或离开该机场，它们的节点就是相邻的，因此不能共享同一个登机口。确定哪些飞机可以在不发生冲突的情况下被安排到哪些登机口，就可以被视为一个着色问题。

一天只有那么多小时，一所学校也只有那么多教室；我们可以把教室和时间段视为颜色。两门不同的课程不能在同一时间、同一教室上课，而且，任何同时选修两门课的学生也不可能在同一时间出现在两个地方。实际上我们还有许多额外约束：化学课不能在计算机实验室上，电影研究需要一间有投影仪的教室，等等。把所有这些约束表示为图中的邻接关系，然后高效地找到一种着色方案，使课程在特定时间安排到特定教室而不发生冲突，这是图论中一个研究得非常充分的问题。

编译器代码生成器中优化团队的开发者，经常需要解决寄存器调度问题。处理器通常不会直接在内存中操作值；相反，它们会把值从内存加载到有限数量的寄存器中，在寄存器中进行计算，然后再把结果存回内存。我们如何才能高效地利用这些有限的寄存器呢？例如：
...
局部变量 i 和 j 理想情况下永远不应该使用同一个寄存器，因为如果它们共享寄存器，其中一个变量就必须被“溢出”——保存到内存中并在之后再加载回来，这样做很慢。我们说这些变量的“生命周期”发生了重叠。但 k 可以使用 i 或 j 使用过的寄存器，因为它的生命周期与它们都不重叠。

寄存器调度问题可以通过如下方式解决：为每个局部变量创建一个节点，为生命周期重叠的变量之间创建一条边，并把寄存器当作颜色。如果能为每个局部变量找到一种颜色，那么我们就以零溢出的方式解决了寄存器调度问题。如果做不到，就需要进一步分析，找出哪个局部变量是溢出代价最低的，移除图中对应的边，然后再次尝试着色。

图着色有许多应用，多到足以写满好几本书。把这个工具放进你的工具箱里，也许在下次你需要解决一个调度问题，或者被一道数独难住时，就能派上用场。

## 8.7 总结

图论是组合数学的一个分支，研究由边连接起来的节点。其中研究得最多的问题之一是节点着色问题：如果我们有一定数量的颜色，并希望给每个节点分配一种颜色，使得任意两个相邻节点的颜色都不相同，我们是否能做到？如果能，又该如何做到？

对于包含许多低度节点的图，通常可以用一种相对简单、近似线性的算法来着色：找出一个可以最后着色的节点，递归地为去掉该节点后的图着色，然后再为最后那个节点选择一种颜色。

更复杂的图可以通过回溯算法来着色：先减少每个节点的可能颜色数量，检查结果是被破坏了、已经得到解了，还是一个部分尝试。如果是部分尝试，就选择一个节点，对该节点的所有可能进行猜测，并递归求解。

回溯算法在结构上与笛卡尔积算法是相同的——都是对序列上的 SelectMany 操作符进行组合——但它多了一个约简步骤，用来尽量避免枚举那些不可能满足所需约束的解空间部分。

一般来说，图着色是一个 NP 完全问题：目前尚不存在已知的多项式时间解法，但也还没有证明这种解法不可能存在。已知的最优通用算法都是指数级的。幸运的是，借助一个好的 reducer，许多实际问题都可以在合理的时间内得到解决。
