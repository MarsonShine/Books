...
**套接字选项（socket options）**是用来控制套接字接口属性。套接字选项可以用来控制套接字的行为和性能；例如超时时间、缓冲区大小、重用地址等。

可以通过调用 `setsocket` 函数设置套接字选项，`getsocketopt` 函数查看套接字选项。

以下是常用的套接字选项：

- `SO_REUSEADDR`：表示是否重用地址。如果设置为 true，则可以在同一个端口上启动多个服务器程序，否则只能启动一个服务器程序。
- `SO_KEEPALIVE`：表示是否开启 TCP keep-alive 机制。true，可以检测到连接是否已经断开；否则只能等到超时时间才能检测到
- `SO_SNDBUF` 和 `SO_RCVBUF`：表示发送缓冲区和接收缓冲区的大小；这是大的缓冲区能减少传输次数以提高网络传输的效率。
- `TCP_NODELAY`：表示是否启用 Nagle 算法。true，可以减少网络传输的延迟，否则需要等待一定时间才能发送数据。
