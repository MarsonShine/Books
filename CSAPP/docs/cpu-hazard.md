...
> 流水线为什么能提高指令的并行度？
>
> 1. 流水线的深度（级数）能增加指令的重叠数，能让更多的指令重叠执行。之前讨论的都是五级流水线，讲一个指令的执行拆分为5个阶段，如果我们拆分出更多的级数，就会有更多的操作可以重叠执行，指令间的并行度更高。同时时钟周期变短，主频变高，处理器性能也就更强了。
> 2. 还有一种提高并行度就是增加硬件了，增加流水线内部的功能部件数量，这样可以每时钟周期发射多个指令。这被称为“多发射（multiple issue）”。高端处理器的发射宽度一般能到3～6条指令，普通处理器能到2条指令。虽然增加时钟周期的指令数，但是也增加了分支预测，指令冒险的复杂度。
>    1. 静态多发射，依靠编译器完成发射相关的判断（如是否发生冒险，分支预测等）
>    2. 动态多发射，在动态执行过程由硬件来完成发射相关的判断
>
> 这种并行被称为指令级并行（ILP）。
...
> 保留站是如何存留多个指令的呢？乱序执行指令又是如何做到的呢？
>
> 保留站中能允许将指令结果直接传递给后续的功能单元而不需要寄存器，这是因为接住一个新增的旁路元件实现的，这个元件就是公共数据总线（CDB）。它允许同时载入所有等待一个操作数的单元。**并且在具有多个执行单元并且每个时钟周期发射多条指令的流水线中**，公共数据总线不止一个。
>
> 前面讲到的乱序直接的过程其实会带来更大的复杂型，因为指令执行的顺序问题，势必会导致RAW以及WAW冒险问题。这一问题主要是通过[Tomasulo算法](https://zh.wikipedia.org/wiki/%E6%89%98%E9%A9%AC%E8%8B%8F%E6%B4%9B%E7%AE%97%E6%B3%95)解决的，其主要思想就是通过**寄存器重命名**来消除这些冒险问题的。
>
> Tomasulo算法的执行过程：第一步从指令队列中读取指令，它是FIFO队列来保持这些数据流的正确性。如果有一个匹配的保留站为空，则将这条指令发送这个站中。如果没有空的保留站就属于结构冒险，就必须要等到有保留站为空/释放。如果这些操作数不在寄存器中，则会追踪将生成这些操作数的功能单元。这一步将会对寄存器进行重命名以消除WAR和WAW冒险。第二部执行，将其可用的操作数以及指令交给功能单元执行。第三步就是写结果到CDB上，由CDB转送给寄存器和任意等待这一结果的存储缓存区中。具体详情可以查阅《计算机体系结构》中的3.4.2小节。

> **寄存器重命名**
>
> 关于寄存器重命名先看下面一个例子（该例子来源于《计算机体系结构》3.4.2节）
>
> ```
> DIV.D				F0,F2,F4
> ADD.D				F6,F0,F8
...
> ```
>
> 这段代码因为F6以及F8存在多处冒险（WAR，WAW）。以下是寄存器重命名之后的汇编代码
>
> ```
> DIV.D				F0,F2,F4
> ADD.D				S,F0,F8
...
> ```
>
> 这样就消除了相关的多处依赖冒险，主要思路就是新增一个中间寄存器，将一个指令的结果写到这个临时寄存器中，只有等这条指令提交的时候再写到结果接存器中。这个重命名过程可以由编译器完成。这样做可以利于分支预测，如果分支预测错了，那么只需要销毁这个临时寄存器就够了。
>
> 这里面还有一个问题，就是如果载入指令和存储指令访问的是不同的地址，那么可以放心乱序执行。但是如果是同一个地址，那么就会出现两种情况：
>
> 1. 根据程序顺序，载入指令位于存储指令之前，交换它们就会导致WAR冒险；
> 2. 根据程序顺序，存储指令位于载入指令之前，交换它们就会导致RAW冒险；
>
> 所以为了检测这两种情况，CPU必须得检测出该载入指令之前的任何未完成的存储指令是否与该载入指令共享相同的数据存储器地址。对于存储指令也是如此。所以要解决这个问题就必须要等到对应的指令都执行完毕才能开始执行。
>
> 具体详细可以查看《计算机体系结构》3.5，3.9章
