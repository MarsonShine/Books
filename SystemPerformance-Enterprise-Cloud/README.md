...
在块 I/O 层的两种调度器这里要提一下，现在绝大多数都是用的多队列调度器。

传统调度器存在一个问题，它们使用一个全局共享请求队列，该队列有一个单独的锁来保护，这个全局锁在高 I/O 的情况下会变成性能瓶颈。

而多队列调度器会针对每个 CPU 使用不同的请求提交队列，并且针对多个设备使用多个分发队列。这样在性能上要比传统调度器好很多。

> 之所以被称为“块 I/O”是因为这些设备都是以“块”为单位访问的。一般以 512 字节的扇区为单位。
...
下图展示了在一般情况下，数据从网卡（NIC）到应用程序之间的流动过程：

![](./asserts/13.png)

在数据网络传输的路径上，还有多种方式绕过一些路径来提高传输性能：
...
应用程序可以使用**数据层开发套件（DPDK）**这样的技术来绕过内核网络软件栈，这样可以提高性能，提高网络包处理能力。这种技术需要应用程序自己在用户态实现自己的网络软件栈，使用 DPDK 软件库和内核用户态 I/O 驱动（UIO）或者虚拟 I/O 驱动（VFIO）来直接向网卡设备驱动程序发送数据。这样可以直接从网卡内存中读取数据包的技术来避免数据的多次复制。

但是这种技术绕过了内核整个网络软件栈，所以就分析问题，获取指标就很困难。

#### XDP 技术

高速数据路径技术（XDP）为网络数据包提供了一条路：可以使用 BPF 技术，直接与现有的内核软件栈集成，这样无需绕过网络软件栈，这样也更好追踪、分析并解决问题。

它是通过网卡驱动程序中提供的 BPF 钩子函数（过滤器）直接访问原始网络数据帧，进而做自定义的处理。

### 网络套接字接收缩放技术

网络套接字接收缩放技术（RSS）它能改善网络接收性能，可以在多 CPU 系统上分散接收数据包的处理，达到更高的网络接收吞吐量。

它是通过网卡硬件实现的：网卡根据数据包的哈希值，将它们分配到多个队列中。每个队列由单独的 CPU 来处理数据包，以达到接收处理并行化。要注意，此技术能提高网络性能，但是也消耗 CPU 资源

### TCP积压队列

当内核接收到客户端的 TCP 连接时，会触发 SYN 包。服务器收到后会将这个连接分配相关的资源并临时存储到一个队列中，这个队列就是**TCP挤压队列（Backlog Queue）**。这样可以有如下作用:

- 由于这个队列只保留非常有效的元数据信息，所以占用的资源非常少，可以防止大量请求的攻击，防止开销过大而崩溃。
- 防止请求丢失，直到调用 accept 为止。
- 减少 TIME_WAIT 状态连接数。

除了挤压队列，还有一个表示完成连接的**监听挤压队列**，这里存放的是握手完成的连接，以供应用程序使用。
