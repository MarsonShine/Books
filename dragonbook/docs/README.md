...
LR技术是一种用于编译器设计和语法分析的技术，它的主要目的是从左到右扫描输入，并根据从右到左（逆向）构建的语法树进行归约。LR技术是解析上下文无关文法（CFG）的强大方法。

LR解析器有多种变体，主要包括：

- **简单LR（SLR）解析器**：使用最简单的LR分析方法，适用于更简单的文法。
- **规范LR（Canonical LR, LR(1)）解析器**：每个状态包含更多的信息（如向前看符号），因此能够处理更复杂的文法。
- **LALR（Look-Ahead LR）解析器**：一种折中的方法，结合了SLR和LR(1)的优点，状态数量较少，适用于实际编译器。

LR解析器主要包括两个表和一个栈：

- **分析表**：包括动作表（Action Table）和转移表（Goto Table）。动作表用于决定当前输入符号和栈顶状态的动作（移进、归约、接受或出错），转移表用于状态转移。
- **状态栈**：用于存储当前解析状态。
- **符号栈**：用于存储已经解析的符号。