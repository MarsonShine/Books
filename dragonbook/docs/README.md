...
在 LR 分析中，一个项目（item）通常表示为一个产生式加上一个点，表示在产生式中的某个位置。例如，产生式 `A -> X Y` 可能会有如下几个项目：

- `A -> . X Y`
- `A -> X . Y`
- `A -> X Y .`

这里的点表示语法分析器已经看到了产生式左侧的一部分，接下来期待右侧的某部分。

**LR(0)项集**：包含了所有可能的项目（状态）集合，不考虑向前看符号。

- 构建 LR(0) 自动机时，首先为每个产生式添加点，生成初始项目集。
- 对每个项目，计算其闭包，表示可以从该项目通过若干步到达的所有可能项目。
- 根据输入符号进行转移，构造状态转换表。

**LR(1) 项集**：是 LR(0) 项集的扩展，引入了向前看符号，增强了区分不同语法上下文的能力。

- 项的形式变为 `[A -> α . β, a]`，其中  `a` 是向前看符号。
- 构建时需要计算每个项目的闭包，考虑向前看符号的传播。

#### 构建项集族

1. **初始项目集**：从文法的开始符号出发，构建初始项目集。
2. **闭包计算**：对于每个项目，添加所有可能的后继项目，直至无法再添加。
3. **GOTO 操作**：对于每个项目集，根据输入符号计算转移后的项目集。
...
2. 计算闭包和转移，形成项集 I1, I2, ... ，直到无法生成新项集。