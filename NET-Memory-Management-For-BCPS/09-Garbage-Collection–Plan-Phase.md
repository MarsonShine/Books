...
图9-13 存在间隙时固定插桩的管理——(a)含单个固定插桩的对象布局示例，(b)插桩信息组织结构，(c)压缩后的结果
...
总结来看，最典型场景是固定对象位于大型已标记对象块内部（图9-18a）。此时必须同时保存前插桩与后插桩数据，并创建三个独立插桩（包含一个扩展后的固定插桩）（图9-18b）。
