...
- `generation`：表示单个代。包含代对应的段/区域信息、分配相关数据及其他元数据（分配指针、起始段等）。
- `heap_segment`：如前所述表示单个段/区域信息。所有段/区域通过单向链表连接，每个段可指向同代别的下一个段。

![](asserts/5-25a.png)

图5-25b. 采用区域模型时，`heap_segment` 实例存储在托管堆之外。这展示了多堆模式的服务器布局（仅显示一个 `gc_heap` 与其代际及对应区域的关系）

运行时启动时，区域会在由全局变量 `g_gc_lowest_address` 和 `g_gc_highest_address` 指向的专属地址空间保留范围内创建。区域初始按类型分组：持久化对象区域（POH）在前，随后是第2代、第1代、第0代区域，大对象区域（LOH）位于末尾。

了解这些机制后，您现在可以理解诸如之前使用的 `GC.GetGeneration` 方法的实现（参见分别基于段模型的清单5-19a和基于区域模型的清单5-19b）。

清单5-19a. 当 `GC.GetGeneration` 方法执行时调用的 `gc_heap` 类方法（段模型实现）

```cpp
// 返回对象的代际编号
// 假设对象是有效的
// 注意：对于LOH对象将返回max_generation
...
清单5-19b. 当 `GC.GetGeneration` 方法执行时调用的 `GCHeap` 和 `gc_heap` 类方法（区域模型实现）

```cpp
// 注意：对于非GC堆中的对象将返回INT32_MAX
int GCHeap::WhichGeneration(Object* object)
{
    uint8_t* o = (uint8_t*)object;
    // 若不在托管堆内，则位于非GC堆(NGCH)
...
// 返回对象的代际编号
// 假设对象是有效的
// 注意：对于UOH对象将返回max_generation
int gc_heap::object_gennum(uint8_t* o)
{
    // 查询内部数据结构，获取地址对应的region_info位域
    // 其中代际信息存储在最低2位
...
在程序执行过程中，可能会创建越来越多的段来容纳所有分配的对象。这就引出了一个问题：段是否会被移除？答案是肯定的。但通常情况是，答案比简单的“是”更为复杂。

首先，我们来看.NET运行时决定移除段的场景。事实上，这种情况只有一个原因——段在垃圾回收后变为空段（完全不包含任何对象）。后续在详细讲解垃圾回收机制时，您会看到这种情况的发生条件。

其次，“移除段”究竟意味着什么？最简单的理解就是调用 `VirtualFree`（或Linux系统中的对应函数）释放段的整个保留地址范围，将内存返还给操作系统。假设出现如图5-26a所示的情况：我们的程序有四个段。由于第2代堆较大，占用了两个段。如前所述，当前已提交的内存（白色区域）比对象实际占用的内存（虚线区域）更多，这是因为内存是预先分配的。经过一段时间后，第2代堆发生了压缩式垃圾回收，其中许多对象被移除（见图5-26b）。实际上回收的空间如此之多，以至于其中一个包含第2代堆的段变成了空段（不包含任何对象）。但此时整个内存仍处于提交状态。最简单的处理方案就是直接释放这部分内存（见图5-26c）。

虽然这个方案看似合理，但它存在一个重要缺陷：持续创建和移除段可能会导致内存碎片问题。这个问题在32位应用程序中尤为严重——因为虚拟内存空间有限，特别是在长期运行的Web应用中。在.NET 2.0和ASP.NET 2.0时代，几乎所有应用程序都是32位的，因此引入了更智能的段处理机制：VM Hoarding（虚拟内存囤积）。其核心理念很简单：与其完全释放空段，不如将其保存（囤积）以供后续重用（见图5-26d）。具体表现为：

- 整个段的内存保持保留状态
- 段的大部分内存会被解除提交（不占用物理内存）——仅保留起始处少量已提交内存，包括段信息
- 段被存入可重用段列表。当需要新段时，会优先检查该列表以寻找可重用的段，随后将其初始化为新的有效段

![](asserts/5-26.png)

图5-26展示了可能的段重用策略示意图

在64位系统中，由于虚拟地址空间大得多，囤积机制的重要性有所降低。但在段创建和销毁非常频繁的动态场景中，重用已保留内存仍然比向系统申请新内存更快。因此即使在64位场景下，该机制仍具价值。

不过，段囤积功能默认是关闭的，因为.NET运行时不愿持有未使用的虚拟内存（即使只是保留状态）。如果运行普通的桌面或控制台.NET应用程序（未使用宿主API），虚拟内存囤积功能会自动禁用。该行为可通过 `GCRetainVM` 设置来覆盖，这个设置可以通过环境变量或Windows注册表配置。此外，托管.NET运行时的进程可以使用 `System.GC.RetainVM` 配置来启用该功能。IIS托管的ASP.NET Web应用程序就默认启用了此功能。若要追踪应用程序中段的创建、销毁及其原因，最简单的方法是使用以下ETW事件（需启用堆栈跟踪）：

- `GCCreateSegment_V1`：显示地址(Address)、大小(Size)、CLR实例ID(ClrInstanceID)和类型(Type)
- `GCFreeSegment_V1`：显示地址(Address)和CLR实例ID(ClrInstanceID)

前文提到的类型(Type)字段包含三种可能值：`SmallObjectHeap`（小对象堆）、`LargeObjectHeap`（大对象堆）或`PinnedObjectHeap`（固定对象堆）。对于 `NGCH` 段，还可能包含前文提及的 `ReadOnlyHeap`（只读堆）值。

### 区域（Region）重用机制

在区域机制中，其运作原理与段（segment）类似。每个堆通过空闲列表数组（.NET源码中的 `region_free_list`）来追踪空置区域：一个列表用于基础小型对象堆（SOH）区域，一个用于大型对象堆（LOH）/固定对象堆（POH）区域，最后一个用于超大型区域。在非并发垃圾回收过程中，空闲区域（即不包含任何被引用对象的区域）会根据其大小被添加到对应的空闲列表。这些空闲区域的已提交内存会保持提交状态，直到本次GC结束时才决定是否解除提交。

当分配过程中需要新区域时（第6章将详述分配机制），系统会优先从空闲列表中选取足够大的可用区域；若没有合适区域，则会在地址空间的保留部分分配新区域。

空闲列表中的区域设有“年龄”属性，每次垃圾回收后该值会递增（上限为99）。在非并发GC结束时，系统会移除年龄超过“20与堆数量两者最大值”的区域：将其移出空闲列表并解除对应内存的提交状态。相应的 `heap_segment` 会被清空，但地址空间保留部分的内存范围永远不会释放。需注意，区域机制仅适用于64位系统——这与32位系统必须释放虚拟内存段的场景不同，因此不存在地址空间不足的问题，这也意味着囤积（hoarding）设置在此不适用。此外，每个堆的gen0和gen1区域末端会部分解除提交。

区域创建或删除时同样会触发 `GCCreateSegment_V1` 和 `GCFreeSegment_V1` 事件。对于固定对象堆（POH）中的区域，其类型值应为 `PinnedObjectHeap（=3）`。

### 本章总结

本章通过大量知识点帮助您深入理解.NET内存管理机制：

1. **内存组织结构**：详细阐释了垃圾回收器管理的物理/逻辑内存结构，并结合前文知识解释了这种设计的底层原因，使您能透彻理解代际划分、小型对象堆、大型对象堆、固定对象堆和非GC堆的由来
2. **核心概念解析**：重点探讨了托管堆与非GC堆的内存组织细节，其中分代垃圾回收等基础概念是理解.NET不可或缺的关键知识。这些代际相关概念几乎出现在所有.NET内存管理场景中，具有极强的实践价值
3. **深度实现原理**：虽然部分内容（如区域年龄机制）实践应用较少，但通过揭示CLR内部实现逻辑，可帮助您掌握GC设计中的特定实现细节
4. **实践案例分析**：本章包含三个典型场景的解决方案示例，从实践角度展示了代际与内存段相关问题的分析方法
