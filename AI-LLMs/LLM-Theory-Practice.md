...
有监督微调（Supervised Finetuning，SFT）又称指令微调（Instruction Finetuning），是指**在已经训练好的语言模型的基础上**，通过使用有标注的特定任务数据进行进一步的微调，从而使得模型具备遵循指令的能力。这些有标记的数据集通常是在目标任务中收集的，例如**分类**、**目标检测**或**情感分析**。在微调过程中，使用这些有标记的数据集来重新训练预训练模型的一部分或全部参数，使其适应目标任务的要求。

这种方法通常能够提高模型在特定任务上的性能，因为预训练模型已经学习到了丰富的特征和知识，而微调可以使模型更好地适应新的数据和任务。

### 提示学习

提示学习是指在自然语言处理任务中使用预定义的提示或指令来引导模型生成期望的输出。在这种方法中，通过在输入数据前面添加一个问题或指令，以指导模型生成与问题相关的响应或执行特定任务。

提示学习完成整个过程可以分为三个阶段：提示添加、答案搜索、答案映射

![](./asserts/25.png)

### 语境学习

语境学习是指根据输入数据的上下文和语境进行学习和理解。这种方法考虑到了文本或语言的语境信息，以便更好地解释和处理含糊不清或多义的语言表达。如向模型输入特定任务的一些具体例子（也称示例（Demonstration））以及要测试的样例，模型可以根据给定的示例续写出测试样例的答案。

### 高效模型微调

大型语言模型的参数数量庞大，对于微调全部参数，需要高昂的计算资源和算力。然而，为了节省成本并提高效率，研究人员提出了多种参数高效的微调方法，这些方法仅训练少量参数以使模型适应下游任务。

高效模型微调通过一系列优化策略和技术，以较少的计算资源和数据量来进行模型微调的方法。这种方法旨在在**资源受限**的情况下实现模型性能的提升。

在高效模型微调通常有以下几个研究方向：

- **参数冻结（Parameter Freezing）**：将预训练模型的一部分或全部参数固定，只微调特定层或模块的参数。这样可以减少微调过程中的计算量和训练时间。
- **低秩适应（Low-Rank Adaptation）**：通过使用低秩逼近技术，将大型语言模型的参数表示为具有较低秩的近似形式。这种低秩表示可以减少参数数量和计算复杂度。
- **数据增强（Data Augmentation）**：通过对训练数据进行变换和扩充，增加训练样本的多样性。这有助于提高模型的泛化能力和适应下游任务的能力。
- **知识蒸馏（Knowledge Distillation）**：使用预训练模型作为**教师模型**，将其知识传递给较小的微调模型。这可以帮助微调模型学习更好的表示和泛化能力。

#### LoRA

LoRA 旨在减少大型语言模型的参数数量和计算复杂度，以适应资源受限的环境。通过使用**低秩逼近技术**，LoRA 将原始的大型语言模型转化为具有较低秩的近似模型，从而减少了存储和计算需求。

> LoRA 的基本思想是通过对预训练模型的参数进行矩阵分解或降维，将其表示为具有较低秩的矩阵形式。这种低秩表示可以有效地减少参数数量，并且在保持模型性能的同时降低了计算复杂度。
>
> 在微调过程中，LoRA 使用低秩逼近的模型作为起点，并根据特定任务的训练数据进行微调。通过这种方式，LoRA 可以在资源受限的环境中实现高效的模型适应。

#### 知识蒸馏

知识蒸馏旨在**将大型模型的知识转移到较小模型，从而使较小模型能够达到与大型模型相似甚至更高的准确性**。

在知识蒸馏中，大型模型被称为“教师网络”，而较小模型被称为“学生网络”。教师网络可以是单个大型模型或一组独立模型的集合，通常在复杂数据源上进行训练，包括低分辨率数据、多领域和多任务数据，或跨模态数据。知识蒸馏的过程可以捕获并转移教师模型的训练知识到较小的学生模型，然后对学生网络进行训练。

教师-学生架构是知识蒸馏的关键组成部分。它包括三种类型的知识蒸馏技术：基于响应的知识蒸馏、基于特征的知识蒸馏和基于关系的知识蒸馏。基于响应的知识蒸馏捕获和转移教师网络输出层（预测）的信息，学生网络直接模仿这些最终预测。基于特征的知识蒸馏捕获教师网络的中间和输出层的信息。基于关系的知识蒸馏探索不同数据样本和层之间的关系。此外，还有离线蒸馏、在线蒸馏和自蒸馏等三种训练方案。

更多关于知识蒸馏可详见：[Knowledge Distillation: Principles, Algorithms, Applications (neptune.ai)](https://neptune.ai/blog/knowledge-distillation)

### 模型上下文窗口拓展

上下文窗口是指语言模型在生成对提示的响应时可以考虑的标记范围。GPT 模型从 2K 窗口大小（GPT-3）开始，一直扩展到32K（GPT-4）。上下文窗口大小的扩展对模型的性能和在各种应用中的实用性都有显著影响。具有更大上下文窗口的语言模型能够处理更长的输入，例如整个文档，或者理解文章的全部内容。这种能力使语言模型能够通过更全面地理解输入来生成更具上下文相关性的响应。

为了更好地满足长文本需求，主要有以下方法来扩展语言模型的长文本建模能力：

- **增加上下文窗口的微调**：采用直接的方式，即通过使用一个更长的上下文窗口来微调现有的预训练 Transformer，以适应长文本建模需求。
- **位置编码**：用于为输入的标记序列中的每个标记分配一个位置。这有助于模型理解标记之间的相对位置关系。
- **插值法**：用于在模型中**动态调整**位置编码（其关键思想是，直接缩小位置索引，使最大位置索引与预训练阶段的上下文窗口限制相匹配），以便模型能够处理更长的上下文。

通过动态调整位置编码，模型可以更好地处理超出其标准上下文窗口大小的输入。这种方法有助于提高模型的性能和实用性，使其能够更好地处理长文本和更复杂的任务。

### 指令构建数据

**手动构建**指令数据集通常涉及人工编写和整理指令，这需要大量的时间和人力资源。这种方法适用于特定领域或特定任务的指令，因为它可以确保数据的准确性和质量。

**自动生成**指令数据集则是利用自然语言处理技术和规则来生成指令。这种方法可以快速生成大量的数据，但可能会牺牲一定的准确性和真实性。

**开源指令**数据集是指已经由其他研究人员或组织公开发布的指令数据集。这些数据集通常经过严格的整理和标注，可以用于各种研究和开发任务。

## 强化学习

强化学习（Reinforcement Learning）旨在通过**智能体**与**环境**的交互，学习如何通过采取不同的动作来最大化累积奖励信号。它涉及定义一个马尔可夫决策过程（Markov Decision Process），包括**状态空间**、**动作空间**、**奖励函数**和**状态转移概率**，以及一个策略用于选择动作。

强化学习由以下几个部分组成：

- **智能体和环境**
- **状态，行为与奖励**：**奖励函数**是其中重要的组成部分：它用于评估智能体采取特定行动后的表现。智能体的目标是最大化长期奖励，而不仅仅是短期奖励。为了实现这一目标，智能体需要学会权衡即时奖励和长期回报，以便做出最佳的决策。
- **策略与价值**：**策略函数**定义了智能体如何根据环境状态来做出决策。**价值函数**它用于估计在给定状态下采取特定行动的长期回报。智能体使用这些价值函数来指导其决策过程，以便选择能够最大化长期奖励的行为。
...
**基于人类反馈的强化学习（Reinforcement Learning from Human Feedback，RLHF）**是一种将人类的专家知识和反馈融入到强化学习过程中的方法。它旨在通过结合人类的智慧和经验，加速智能体的学习过程并提高其性能。主要体现在 3H（帮助性（Helpfulness）、真实性（Honesty）以及无害性（Harmless））方面。

主要分为两个主要步骤：**奖励模型训练**和**近端策略优化**。

1. 奖励模型训练：

   奖励模型通过由人类反馈标注（这些反馈信号可以是奖励信号、评分、喜好度等形式，用于指导智能体在学习过程中的行为）的偏好数据来学习人类的偏好，判断模型回复的有用性以及保证内容的无害性。奖励模型模拟了人类的偏好信息，能够不断地为模型的训练提供奖励信号。在获得奖励模型后，需要借助强化学习对语言模型继续进行微调。

2. **近端策略优化（Proximal Policy Optimization, PPO）**：

   在近端策略优化阶段，智能体使用经过奖励模型训练的奖励模型作为指导，通过与环境的交互来优化其策略。策略是智能体在特定状态下选择动作的规则或策略函数。

   近端策略优化涉及到四个模型：

   - 策略模型（Policy Model），生成模型回复。
   - 奖励模型（Reward Model），输出奖励分数来评估回复质量的好坏。
   - 评论模型（Critic Model），来预测回复的好坏，可以在训练过程中实时调整模型，选择对未来累积收益最大的行为。
   - 参考模型（Reference Model）提供了一个 SFT 模型的备份，帮助模型不会出现过于极端的变化。
   ...
   3. 优化调整：使用**优势函数**来优化和调整策略模型，同时利用参考模型确保更新的策略不会有太大的变化，从而维持模型的稳定性。

## 评估大模型

随着 ChatGPT 于 2022 年 11 月底发布以来，后面问世的大模型呈井喷式增长。因此如何评估大语言模型变得愈发关键。我们需要在**技术**和**任务**层面对大语言模型之间的优劣加以判断，同时也需要在社会层面对大语言模型可能带来的潜在风险进行评估。