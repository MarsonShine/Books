...
> 为了让 **GC / 异常 / 调试器** 能“按真实调用顺序”遍历线程栈，既要处理 **托管方法帧（managed frame）**，也要处理 **运行时的非托管代码帧（unmanaged C++ frame）**。两者的“可遍历性”来源不同，所以 CLR用了两套机制，并把它们在遍历时“合并排序”。
>
> ## 托管帧（Managed frame）：靠 JIT 元数据来“虚拟回退”
>
> 托管代码（C#/IL）最终由 JIT 编译成本机代码。因为 JIT 是运行时控制的，所以 CLR可以要求 JIT：
>
> - 不一定让每个方法都用固定的栈帧格式（比如 x86 系统的 push ebp; mov ebp, esp; ...）
>   - 固定格式好处：容易回溯；
>   - 坏处：对很多小方法（尤其叶子函数/属性 getter）会引入额外指令和寄存器/栈操作，影响性能。
>
> 因此 CLR 选择了性能更好的方案：
>
> - 每个被 JIT 的方法都会生成一份“可回溯元数据”（unwind info / GC info）
>
>   这份信息描述：在某个指令位置（IP）下，
>
>   - 栈指针怎么变化了（SP/ESP/RSP）
>   - 哪些寄存器被保存到了哪里
>   - 哪些栈槽/寄存器里可能放着托管对象引用（给 GC 用）
>   - 怎么从当前点“还原”到调用者的寄存器状态
>
> 堆栈遍历器拿到少数关键寄存器初值（典型如：IP、SP，有时还要 FP/EBP）后：
>
> 1. 用 IP 查到“它属于哪个 JIT 方法”
> 2. 找到该方法的 JIT 元数据
> 3. 根据元数据计算出“调用者的 IP/SP/寄存器值”（这一步叫 **virtual unwind 虚拟回退**）
> 4. 重复，就能得到一串托管调用链
>
> 强调“虚拟”是因为：只是计算“如果退栈会是怎么样”，**不真的改 CPU 寄存器**，避免破坏现场。
>
> ## 运行时非托管帧：C++ 没法靠 JIT 元数据统一回退，所以用 Frame 链表显式标注
>
> 运行时本身（coreclr.dll 里大量 VM 代码）是 **非托管 C++**：
>
> - C++ 编译器生成的栈帧形态很多样，CLR不能像控制 JIT 那样强制每个 C++ 函数都“可遍历”且包含 GC/异常所需信息。
> - 但运行时有些 C++ 函数又确实“很重要”：
>   例如它在局部变量里暂存了托管对象引用；或者它处于托管/非托管过渡点，需要异常处理信息。
>
> 所以 CLR 的策略不是“让所有非托管帧都可遍历”，而是：
>
> - **只有那些需要在栈遍历时被识别/报告信息的 C++ 代码段，显式在栈上放一个 `Frame` 对象**（一个 C++ 结构体/类实例）
> - 这些 Frame形成一个单向链表：
>   - 每个 `Frame` 有 `next` 指针指向更老的 `Frame`
>   - `Thread` 结构保存链表表头（最新的 Frame）
>   - 进入某段需要被识别的运行时逻辑：push 一个 Frame
>   - 离开：pop 掉 Frame
>
> 注意这里的“Frame”是特指 **CLR 的 Frame 数据结构**（大写 Frame），不是“CPU 栈帧”的泛称。
>
> ## 关键点：托管帧和 Frame 会交错出现，遍历时必须按“栈地址顺序”合并
>
> 真实调用链可能长这样（从新到旧）：
>
> - 托管方法 M3（JIT）
> - 进入运行时：C++ 函数 R1（期间 push 了一个或多个 Frame）
> - 运行时又回调托管：M2
> - 又进入运行时：R2（push Frame）
> - 托管方法 M1
> - ...
>
> 如果你“先把所有托管帧遍历完，再遍历所有 Frame 链表”，顺序会错。
>
> 为了解决这个问题，CLR对 Frame 施加了一个非常重要的约束：
>
> - **Frame 对象必须分配在“push 它的那个函数”的栈上**（也就是 Frame 的内存地址就在当前线程的栈区间里）
>
> 这样堆栈遍历器在任意时刻都有两个“下一帧候选”：
>
> 1. 通过 JIT 元数据 virtual unwind 得到的“下一个托管帧”（更老的 managed frame）
> 2. Frame 链表里的“下一个更老的 Frame”
>
> 然后用一个很朴素的规则决定选哪个：
>
> - **比较这两个候选对象在栈上的位置，哪个更靠近栈顶（更新），就先返回哪个**
>   （具体是比地址大小还是边界，取决于栈是向下还是向上增长；多数平台栈向下增长，所以“更靠近栈顶”通常意味着地址更大。）
>
> 因此遍历能得到一个正确交错的序列。
>
> ## 过渡 Frame（transition Frame）：托管 ↔ 非托管 边界的“连接器”
>
> 当托管代码调用进入运行时非托管代码时，运行时通常会压入某种 **transition Frame**，主要目的两类：
>
> 1. **保存托管调用点的寄存器/上下文**
>    这样当遍历器走完非托管 Frame 后，还能“接回去”继续 unwinding 托管帧。
> 2. **向 GC 报告托管对象引用**
>    因为托管对象（引用）可能作为参数传给运行时 C++ 函数，或者暂存在某个地方。发生 GC 时必须能找到这些引用，避免对象被错误回收/移动后引用失效。
>
> 总的来说：
>
> - 托管方法：**JIT 负责生成可回溯元数据** → 遍历器靠 virtual unwind 还原调用者。
> - 运行时非托管方法：不能指望编译器统一可回溯 → **需要时显式在栈上 push 一个 CLR 的 Frame**，并挂到 Thread 的 Frame 链表。
> - 遍历时：同时看“下一个托管帧”和“下一个 Frame”，用栈地址比较把两者合并成真实调用顺序。
