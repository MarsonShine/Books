...
> **“形式化”（Formalize / Formalizing）** 的意思并不是指某种数学证明，而是指：
>
> **将原本“非正式、隐含、紧耦合”的内部实现关系，转变为“显式、规范、严格定义”的标准接口或协议。**
>
> 为了让你透彻理解，我们可以从以下几个层面拆解这个词的含义：
>
> ### 1. 从“默契”转变为“规范”
>
> 在目前的 .NET 实现中，CLR（运行时）和 CoreLib（核心库，如 `System.Private.CoreLib.dll`）是**同步开发、同步编译、版本强绑定**的。
>
> - **目前的非形式化状态：** CLR 内部硬编码了对 CoreLib 中某个类（比如 `String` 或 `Thread`）内部字段偏移量的了解。CLR 知道“这个字段在第 12 个字节”，因为它和 CoreLib 是一起编译出来的。这是一种“内部默契”。
> - **形式化之后：** 如果要加载多个 CoreLib，CLR 就不能再靠“默契”了。必须写下一份**正式的、文档化的标准**（即“契约”），规定：不管你是哪个版本的 CoreLib，`String` 类的内存布局必须长什么样。这就叫形式化。
>
> ### 2. 定义稳定的 ABI（二进制接口）
>
> 形式化契约通常意味着定义一套 **ABI (Application Binary Interface)**。
>
> - **FCall 方法的形式化：** FCall 是托管代码直接调用 C++ 运行时代码的机制。目前它们的关联非常隐晦。形式化意味着要明确定义：调用参数怎么传、返回值怎么处理、异常怎么抛，且这些定义在不同版本之间不能随便改。
> - **数据类型布局的形式化：** 明确规定基础类型（Object, String, Array, Exception）在内存里的每一位代表什么，形成一个跨版本通用的“模版”。
>
> ### 3. 解耦（Decoupling）
>
> 形式化是一个**去耦合**的过程。
>
> - **现状：** CLR 和 CoreLib 像连体婴儿。
> - **形式化后：** CLR 像是一个插座，CoreLib 像是一个插头。只要插头符合“形式化”定义的标准（尺寸、电压、电流规格），不管是 .NET 6 的插头还是 .NET 9 的插头，都能插在这个 CLR 上运行。
...
CLR 内部提供了一个 [`mscorlib` binder](https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/binder.cpp)，用于在非托管类型与字段和托管类型与字段之间建立映射。binder 会查找并加载类，并允许调用托管方法。它还会对托管与本机代码中指定的布局信息做简单校验，以保证其正确性。binder 会确保尝试加载的托管类存在于 mscorlib 中、已被加载，并且字段偏移是正确的；它还需要能够区分不同签名的重载方法。
...
class Foo
{
    ...
    public string Bar(int flags)
    {
        ...
        // 字符串从 QCall 返回时，是通过使用 StringHandleOnStack
        // 获取本地变量地址的方式返回的
...
QCall 入口点需要在 [vm\qcallentrypoints.cpp][https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/qcallentrypoints.cpp] 的表里通过 `DllImportEntry` 宏注册。参见下方 ["Registering your QCall or FCall Method"](#register)。

[qcall-entrypoints]: https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/qcallentrypoints.cpp

```C++
// 所有 QCall 都应该是自由函数（free function），并标记为 QCALLTYPE 以及 extern "C"
extern "C" BOOL QCALLTYPE Foo_BarInternal(int flags, LPCWSTR wszString, QCall::StringHandleOnStack retString)
{
    // 所有 QCall 都应该有 QCALL_CONTRACT。
    // 它是 THROWS; GC_TRIGGERS; MODE_PREEMPTIVE 的别名。
    QCALL_CONTRACT;

    // 如果你想指定前置条件，也可以改用 QCALL_CHECK 以及展开形式的契约：
    // CONTRACTL {
    //     QCALL_CHECK;
    //     PRECONDITION(wszString != NULL);
    // } CONTRACTL_END;

    // QCALL_CONTRACT 和 BEGIN_QCALL 之间唯一允许出现的一行
    // 应该是（如果有的话）返回值的声明。
    BOOL retVal = FALSE;

    // 函数体必须被 BEGIN_QCALL/END_QCALL 宏包起来。
    // 这对异常处理是必要的。
    BEGIN_QCALL;

    // 参数验证理想情况下应在托管侧完成，但在某些情况下
    // 需要在本机侧完成。如果参数验证在托管侧完成了，
    // 那么在本机侧进行断言是合理的。
    _ASSERTE(flags != 0);

    // 不用担心传入 QCall 的字符串会被 GC 移动。
    // 封送（marshalling）会为我们把它们固定住（pin）。
    printf("%S\n", wszString);
    ...
    // 不能在 BEGIN_QCALL/END_QCALL 内部直接 return。
    // 返回值必须通过辅助变量传递出来。
    retVal = TRUE;
...
FCall 入口点需要在 [vm\ecalllist.h][https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/ecalllist.h] 中通过 `FCFuncEntry` 宏在表里注册。参见 ["Registering your QCall or FCall Method"](#register)。
...
CLR 必须知道你的 QCall 和 FCall 方法的名字：既包括托管侧的类名与方法名，也包括要调用的本机方法。对 FCall 而言，注册在 [ecalllist.h][https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/ecalllist.h] 中完成，涉及两个数组。第一个数组把命名空间与类名映射到一个“函数元素数组”。该函数元素数组再把具体的方法名与签名映射到函数指针。
...
QCall 则在 [qcallentrypoints.cpp][https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/qcallentrypoints.cpp] 的 `s_QCall` 数组中通过 `DllImportEntry` 宏注册，如下所示：
...
在 [corelib.h][https://github.com/dotnet/runtime/blob/main/src/coreclr/vm/corelib.h] 中，使用以 `"_U"` 结尾的宏来描述某个类型、托管代码中的字段名，以及对应本机数据结构中的字段名。此外，你还可以指定一个方法列表，并在之后尝试调用这些方法时按名称引用它们。
...
> ### 1. 查找目标方法（Metadata Lookup）
>
> ```
> MethodDescCallSite releaseHandle(s_pReleaseHandleMethod, METHOD__SAFE_HANDLE__RELEASE_HANDLE, (OBJECTREF*)&sh, TypeHandle(), TRUE);
> ```
>
> - **`s_pReleaseHandleMethod`**: 这是一个指向 `MethodDesc` 的指针。`MethodDesc` 是运行时内部描述一个方法的所有元数据（如签名、地址、所属类）的核心数据结构。
> - **`METHOD__SAFE_HANDLE__RELEASE_HANDLE`**: 这是一个预定义的常量 ID，对应托管代码中 `SafeHandle.ReleaseHandle()` 方法。
> - **`MethodDescCallSite`**: 这是一个辅助类。它的作用是为调用做准备，比如检查方法是否需要 JIT 编译，或者处理虚方法分派（Virtual Dispatch）。
>
> ### 2. 保护托管对象（GC Protection）
>
> 在 C++ 代码中直接持有托管对象是非常危险的，因为一旦触发 GC，对象地址会改变。
>
> ```
> SAFEHANDLEREF sh(psh); // 将原始指针包装成强引用的句柄
> GCPROTECT_BEGIN(sh);   // 告诉 GC：我这手里有个引用，如果你挪了位置，记得更新 sh 变量的地址
> ```
>
> - 因为接下来的 `Call_RetBool` 可能会触发 GC（`GC_TRIGGERS`），所以必须使用 `GCPROTECT` 宏来防止对象在调用过程中被回收或移动。
>
> ### 3. 准备参数（Argument Marshalling）
>
> 托管方法的调用需要遵循特定的调用约定（Calling Convention）。
>
> ```
> ARG_SLOT releaseArgs[] = { ObjToArgSlot(sh) };
> ```
>
> - **`ARG_SLOT`**: 这是运行时内部通用的参数槽位类型（通常是一个 64 位宽的数值）。
> - **`ObjToArgSlot(sh)`**: 把托管对象的引用 `sh` 转换成能通过栈或寄存器传递的格式。
> - 由于 `ReleaseHandle` 是实例方法，这个 `sh` 实际上是作为 `this` 指针传递的。
>
> ### 4. 执行调用（The Execution）
>
> ```
> if (!(BOOL)releaseHandle.Call_RetBool(releaseArgs)) { ... }
> ```
...
> 这是最核心的一步：
>
> 1. **切换状态**：运行时会将当前线程状态从 `Preemptive`（抢占式）切换回 `Cooperative`（协作式），或者建立一个 **Transition Frame**（转换栈帧）。
> 2. **跳转执行**：程序计数器跳转到 `ReleaseHandle` 方法的机器码地址（如果没编译，会先触发 JIT）。
> 3. **捕获异常**：如果托管代码抛出了异常，这个 `Call` 方法内部会处理异常的传播。
> 4. **获取返回值**：`Call_RetBool` 表示我们期望托管方法返回一个布尔值。
...
> 这是理解两者的钥匙：
>
> - FCall 运行在协作模式（Cooperative Mode）：
>   - 在这种模式下，托管线程告诉 GC：“我现在正在执行关键操作，**你不能在我不允许的情况下开始垃圾回收**。”
>   - **后果**：如果 FCall 执行时间过长（比如一个大循环），GC 就必须一直等待该线程到达一个“安全点”（Safe Point）。这就是你提到的“饥饿问题”。
> - QCall 运行在抢占模式（Preemptive Mode）：
>   - 在进入 QCall 之前，线程状态会切换。它告诉 GC：“我现在要去执行非托管代码了，**你可以随时进行垃圾回收**，不用等我。”
>   - **后果**：GC 可以自由运行，不会被阻塞。
>
> ### 2. FCall 的复杂性：样板代码与安全性
>
> FCall 之所以“快”，是因为它几乎没有模式切换的开销。但代价是开发者必须手动维护 GC 的安全性。
>
> #### A. 显式 GC 轮询（GC Poll）
>
> 如果 FCall 内部有循环，你必须显式调用 `FC_GC_POLL` 宏。
>
> ```c++
> FCIMPL1(void, SomeFCall, Object* obj) {
>     for(int i=0; i<10000; i++) {
>         // 每隔一段时间检查一次 GC 是否请求挂起线程
>         FC_GC_POLL_RET(); 
>         // 执行逻辑
>     }
> }
...
> ```
>
> 如果不加这个，在高并发或内存压力下，整个进程可能会因为这个循环而卡死（等待 GC）。
>
> #### B. 保护对象引用（OBJECTREF）
>
> 在 FCall 中，你直接操作内存指针。如果你的 FCall 内部调用了其他可能触发 GC 的函数，你必须使用极其繁琐的宏来“保护”你的对象指针，否则 GC 可能会在运行期间移动对象，导致你的指针变成野指针。
>
> - 样板代码示例：
>
>   ```C++
>   HELPER_METHOD_FRAME_BEGIN_RET_1(obj); // 建立一个特殊的栈帧保护 obj
>   // ... 调用可能触发 GC 的代码 ...
>   HELPER_METHOD_FRAME_END();            // 销毁栈帧
>   ```
>
> #### C. 返回对象的特殊处理
>
> FCall 不能简单地返回一个指针。为了让 JIT 知道如何处理返回值的 GC 句柄，必须使用特殊的返回宏：
>
> - `FC_RETURN_OBJ(obj)`
> - `FC_INNER_RETURN(T, val)`
>
> ------
>
> ### 3. QCall 的设计：安全优先
>
> QCall 是为了解决 FCall 的易错性而诞生的。它是目前 .NET 运行时首选的内部调用方式。
>
> - **参数传递**：QCall 不直接传递对象指针，而是传递 **`ObjectHandleOnStack`**。这是一个指向对象句柄的指针。即使 GC 移动了对象，句柄指向的地址也会被更新，C++ 端始终能拿到正确的地址。
> - **模式切换**：进入 QCall 时，底层会自动处理从协作模式到抢占模式的转换。
> - **代码简洁**：不需要 `HELPER_METHOD_FRAME` 或 `GC_POLL`。
